        ...
        child: BlocConsumer<QiblaCubit, QiblaState>(
          ...
          builder: (context, state) {
            ...
            if (state is QiblaSuccess) {
              return _QiblaContent(state: state, visualAngle: _visualAngle);
            }
  ...
  Widget build(BuildContext context) {
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        shape: BoxShape.circle,
        color: isAligned
            ? AppColors.qiblaCompassRingAligned.withValues(alpha: 0.14)
            : Colors.transparent,
        border: Border.all(
          color: isAligned
              ? AppColors.qiblaCompassRingAligned
        ...
        boxShadow: isAligned
            ? [
                BoxShadow(
                  color: AppColors.qiblaCompassRingAligned.withValues(
                    alpha: 0.28,
                  ),
                  blurRadius: 14,
                ),
      ...
      child: Icon(
        isAligned ? Icons.check_rounded : Icons.my_location_outlined,
        size: 21,
        color: isAligned
            ? AppColors.qiblaCompassRingAligned
            : AppColors.darkSecondaryText,
...
class _QiblaError extends StatelessWidget {
  ...
  String _messageFor(QiblaErrorKey key) {
    switch (key) {
